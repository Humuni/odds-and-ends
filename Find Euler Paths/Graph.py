...
class Graph:
    ...
    def tryVisiting(self, vertex, unvisitedEs, walk):
        """
        ...
        Parameters:
            ...
            int unvisitedEs: total number of unvisited edges so far
            Walk walk: Euler walk built so far

        Returns: True iff an Euler circuit has been found and False otherwise
        """
        # if there are no more unvisited edges corresponding to vertex, check if walk results in a circuit and if all edges have been visited
        # remove if list comparison is possible on moon
        if self.checkTerminal(vertex):
            if (unvisitedEs == 0) and (walk.isCircuit()):
                    return True
            else:
                # no Euler circuit, go back to previous vertex and try another edge
                return False
        # vertex has unvisited edges, try an edge
        else:
            # try to travel through edges that are unvisited
            for i in range(self.totalVertices()):
                if self.visitedE[vertex][i] != self.edges[vertex][i]:
                    # mark as visited
                    # take the edge to next vertex
                    self.visitedE[vertex][i] += 1
                    # catch case for if graph provided is a directed graph
                    if (not self.directed) and (i != vertex):
                        self.visitedE[i][vertex] += 1
                    walk.addVertex(i)
                    # after visiting next vertex, realize it does not result in a Euler circuit, go back to prev
                    if not self.tryVisiting(i, unvisitedEs -1, walk):
                        walk.removeLastVertex()
                        self.visitedE[vertex][i] -= 1
                        if not self.directed and (i != vertex):
                            self.visitedE[i][vertex] -= 1
                    # vertex may be part of the Euler circuit, continue
                    else:
                        return True
            # if all possible edges are visited and does not result in Euler circuit, go back to prev
            return False
    
            
    def checkTerminal(self, vertex):
        ''' Helper function for tryVisiting. Checks if all neighboring edges have been visited

        Parameters:
            vertex (int)

        Returns: True iff all neighboring edges have been visited
        '''
        for i in range(self.totalVertices()):
            # there is still edges that have not been visited
            if (self.visitedE[vertex][i] != self.edges[vertex][i]):
                return False
        return True

# testing stuff(?????????????)
g = Graph.fromFile("medium.txt")
for i in g:
    print(i)
    for total in range(i.totalVertices()):
        w = Walk(i.totalEdges())
        print(i.tryVisiting(total, i.totalEdges(), w))